...
<body>
	<form role='form' action="#" id="noteAction">
		<div id='form-submit'>
			...
			<input type='hidden' name='keyword' value="${cri.keyword}"> <input type="hidden" name='noteFlag' value='sender' />
		</div>
	</form>
	<div class="userInfo">
		<div role="tabpanel">
			...
			<ul class="nav nav-tabs" role="tablist">
				...
				<li role="presentation" class="active"><a href="/note/noteListSender">보낸 쪽지</a></li>
		...
		<div class='pag'>
			<ul class="pagination">
				<c:if test="${pageMaker.prev }">
					<li><a href="/note/noteListSender${pageMaker.makeNotice(pageMaker.startPage -1) }">&laquo;</a></li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
					<li <c:out value="${pageMaker.cri.page == idx?'class =active':''}" />><a href="/note/noteListSender${pageMaker.makeNotice(idx) }">${idx }</a></li>
				</c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
					<li><a href="/note/noteListSender${pageMaker.makeNotice(pageMaker.endPage + 1) }">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	</div>
