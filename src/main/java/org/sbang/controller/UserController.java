...
public class UserController {
	...
	public void regUserGet() throws Exception { // 회원가입창
	}

	@RequestMapping(value = "/regUser", method = RequestMethod.POST)
	public String regUserPost(UserVO userVO, RedirectAttributes rttr) throws Exception { // 회원가입 post

		userService.create(userVO); // 서비스 작동
		rttr.addFlashAttribute("msg", "regSuccess"); // 회원가입 완료 메시지
		rttr.addFlashAttribute("userEmail", userVO.getUserEmail()); // "test@naver.com"님 회원가입을 축하합니다. 메시지 띄울때 사용
		return "redirect:/";
	}
	...
	public String myPageGet(HttpSession session, Model model) throws Exception { // 마이페이지 창
		...
		UserVO userVO = (UserVO) session.getAttribute("login");
		model.addAttribute("UserVO", userService.read(userVO.getUserNo())); // 유저정보 뿌리기
		return "/user/myPage";
	...
	public String myPagePost(UserVO userVO, RedirectAttributes rttr, HttpSession session) throws Exception { // 프로필정보수정
		...
		UserVO tempVO = (UserVO) session.getAttribute("login"); // 세션 가져오기
		userVO.setUserNo(tempVO.getUserNo()); // 세션에 있는 유저 번호 set
		userService.modify(userVO); // 유저정보 수정
