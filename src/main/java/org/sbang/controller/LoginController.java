...
public class LoginController {
	...
	public String loginGet(HttpServletRequest request) throws Exception { // 로그인창
		...
		if (request.getRequestURI() != null && request.getHeader("referer") != null) { // 바로 로그인 URI 접근시 null값 방지
			...
			else if (!(request.getRequestURI().equals("/login") && request.getHeader("referer").equals("http://www.open-study.xyz/login"))) // 로그인창 여러번 할때는 저장하지 않음
	...
	public String kakao(Model model) throws Exception { // 카카오 로그인
		model.addAttribute("url", KakaoLogin.kakaoURL());
